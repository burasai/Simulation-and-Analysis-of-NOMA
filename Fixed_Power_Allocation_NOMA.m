...
x = sqrt(pt)*(sqrt(a1(u))*x1 + sqrt(a2(u))*x2);% NOMA signal
...
y2_est = x2_eq - sqrt(a1(u)) * sqrt(pt) * x12_est;
x2_est = zeros(1, N);
x2_est(y2_est>0) = 1; 
...
semilogy(a1 , BER2,'-b','linewidth',1.5); hold on
xlabel('\alpha_1')
ylabel('BER')
legend('Far user, P = 20 dBm','Near user, 20 dBm')
grid


figure
semilogy(a2 , BER1,'-r','linewidth',1.5); hold on
semilogy(a2 , BER2,'-b','linewidth',1.5); hold on
xlabel('\alpha_2')
ylabel('BER')
legend('Far user, P = 20 dBm','Near user, 20 dBm')
grid
